...
def upload(path):
    ...
    try:
        ...
        print(f"Video ID : {u['file_id']}")
        print(f"Video Url : {u['file_url']}")
        print("#"*40)
    except TypeError:
        print(f"Unsopported video format for {str(path).split('/')[-1]}")
        print("Supported video format : mkv, mp4, wmv, avi, mpeg4, mpegps, flv, 3gp, webm, mov, mpg, m4v")
        sys.exit()
...
def remote(direct_link):
    ...
    if "http://" in direct_link or "https://" in direct_link:
        ...
        print("")
        print("Please see your doodstream remote upload dashboard for further information")
        print("#"*40)
    else:
        print("#"*10 + " Remote Upload " + "#"*10)
